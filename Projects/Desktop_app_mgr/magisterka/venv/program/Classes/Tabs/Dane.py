...
from program.Classes.HelperClass.ComboBoxClass import ComboBoxClass
...
class Dane():
    ...
    def getListOfEntryField(self, framex):
        ...
        for var in dw.listOfElements:
            ...
            entryCombo = ComboBoxClass(framex, dw.listOfElements[var]['zmiennaPola'], dw.listOfElements[var]["opis"],
                               dw.listOfElements[var]["row"], dw.listOfElements[var]["columne"],dw.listOfElements[var]["flag"])
            entry.entryField()
            cbox=entryCombo.entryCombobox(dw.listOfElements[var]["units"],dw.listOfElements[var]['zmiennaPola'])
            cbox.bind("<<ComboboxSelected>>",print("test"))
            # self.cbox=entryUnit.entryUnitsField(dw.listOfElements[var]['units'],dw.listOfElements[var]['zmiennaPola'])
            #cbox.bind("<<ComboboxSelected>>",print("wywoluje po kliknieci"))
            # self.cbox.bind('<Button-1>',print("asas"))
    ...
    def strMasPar(self, _Vzas):
        mzas = (_Vzas * dw._gestWody.get()) / (dw._czasNap.get())
        dw._mzas.set("{:.2f}".format(float(mzas)))
        return mzas
    ...
    def stan(self,*args,widget=None):
        ...
        if(widget.get()=='Wczytaj dane z pliku'):
         ...
         ase.wypelnijDaneExclem()
            ...
            print(name)
     # def comboBox(self):
     #     self.cbox.bind("ComboboxSelected>>",print("lumpalumpy"))